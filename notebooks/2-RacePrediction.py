...
# Directories where the raw CSV and processed data live
...
### Model raining parameters centralization
epoch_setup = 20
step_size_setup = 2
gamma_setup = 0.5
loss_max_nom_setup = 1.0
lstm_hiddens_setup = 1200
lstm_layers_setup = 2
dropout_setup = 0.4
early_stop_patience_setup = 100
lr_setup = 1e-4
weight_decay_setup = 1e-5
...
years = range(start_year, test_year)   #range(2001, 2024)
for year in years:
    ...
    for counter, csv_path in enumerate(sorted(race_dir.iterdir())):
        ...
        for i in range(nlaps):
            ...
            for k in range(0, len(blocks), 7):
                # ─── Bannière avant modification ───
                # 1) driverId → embedding index → one-hot (130)
                raw_id = blocks[k]
                if np.isnan(raw_id):
                    idx = 0
                else:
                    idx = driver_embed_idx(int(raw_id))
                cur_ret = np.append(cur_ret, driver_embed(idx))
                # ─── Bannière après modification ───
                ...
                if np.isnan(raw_st):
                    ...
                    st_id = int(raw_st)
                cur_ret = np.append(cur_ret, status_embed(st_id))
                ...
                if np.isnan(lt):
                    ...
                    lt_val = float(lt)
                cur_ret = np.append(cur_ret, lapTime_embed(lt_val))
        ...
        for i in range(nlaps):
            ...
            for k in range(0, len(blocks), 7):
                ...
                if np.isnan(raw_s2):
                    ...
                    sid2 = int(raw_s2)
                out_vec.extend(status_embed(sid2).tolist())
                raw_lt2 = blocks[k+6]
                if np.isnan(raw_lt2):
                    lt2 = 0.0
                else:
                    lt2 = float(raw_lt2)
                out_vec.extend(lapTime_embed(lt2).tolist())
...
def out_to_in(in_, out_, random=False, num_of_laps=50):
    ...
    for i in range(0, 20):
        _ret[131 + i*196 + 135 : 131 + i*196 + 195] = _o[j*60 : (j+1) * 60]
        # pred_norm_pit = _o[j*60 + 21] # the 22nd float in the 60`
        pit_norm = _o[j*60 + 21]
        if np.isnan(pit_norm):
            pit_norm = 0.0
        if (round(pit_norm * num_of_laps) <= 1):
...
# Trains model on data from 2010 to 2023, 2024 is reserved for testing
# Method 1
# Training procedure:
#   for each race:
#     set zero states,
...
#   - Output features: 2
#   - Dropout: 0.2
model = RacePredictionModel(4051, 1200, lstm_hiddens_setup, lstm_layers_setup, dropout_setup).to(device)

# Mean Squared Error (MSE) loss module is a standard regression loss function
crit     = nn.MSELoss().to(device)
# Mean Absolute Error (MAE) loss measures the average absolute difference
mae_crit = nn.L1Loss().to(device)

# Adam : default optimizer with learning rate 0.001
opt = optim.Adam(model.parameters(), lr=lr_setup, weight_decay = weight_decay_setup)

#   - patience: number of epochs with no improvement before reducing LR
#   - factor: multiplicative factor for decay
# Replace StepLR with ReduceLROnPlateau to decay LR when test loss plateaus
sched = optim.lr_scheduler.ReduceLROnPlateau(
    opt,
    mode='min',
    factor=gamma_setup,
    patience=step_size_setup
    # verbose parameter removed since not supported in this PyTorch version
)
...
def run_train(model, ds, crit, opt, sched, device):
    ...
    # Build list of (year, round) tuples for all races prior to test_year
    race_list = []
    ...
    for (y, rnd) in tqdm(race_list, desc="Training epoch", unit="race"):
        ...
        for inp_batch, exp_batch in race_loader:
            ...
            if torch.isnan(x).any() or torch.isnan(y).any():
                raise RuntimeError("Found NaN in inputs or targets")
            out, st = model(x, st)
            if torch.isnan(out).any():
                raise RuntimeError("Model output contains NaNs")

            opt.zero_grad()
            out, st = model(x, st)
            loss = crit(out, y)
            mae  = mae_crit(out, y)          # compute MAE for monitoring absolute error
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=loss_max_nom_setup)
            opt.step()

            st = (st[0].detach(), st[1].detach())

            total_loss += loss.item()
            total_mae  += mae.item()         # accumulate MAE value
            count      += 1

    # sched.step()  # no longer used here; LR scheduling now driven by validation loss
    return (total_loss / count) if count > 0 else float('nan'), \
           (total_mae  / count) if count > 0 else float('nan')
...
def run_test(model, ds, crit, device):
    ...
    test_years = [y for y in ds.available_years if y >= test_year]
    if test_years:
        year0 = test_years[0]
    else:
        year0 = ds.available_years[-1]
    ds.set_year(year0)
    ds.set_round(1)
    ...
    with torch.no_grad():
        for inp_batch, exp_batch in race_loader:
            ...
            out, st = model(x, st)
            loss = crit(out, y)
            mae  = mae_crit(out, y)         # compute MAE on test set
            total_loss += loss.item()
            total_mae  += mae.item()        # accumulate test MAE
            count      += 1

    return (total_loss / count) if count > 0 else float('nan'), \
           (total_mae  / count) if count > 0 else float('nan')

def run_all(model, ds, crit, opt, sched, device, epochs=epoch_setup):
    train_losses = []
    test_losses  = []
    train_maes   = []
    test_maes    = []
    pit_accs     = []   # placeholder if you compute pit-stop accuracy

    best_te_mae       = float('inf')
    epochs_no_improve = 0
    early_stop_patience = early_stop_patience_setup

    for epoch in range(epochs):
        print(f"Epoch {epoch}")
        tr_loss, tr_mae = run_train(model, ds, crit, opt, sched, device)
        te_loss, te_mae = run_test(model, ds, crit, device)
        ...
        # pit_accs.append(current_pit_accuracy)  # if implemented

        sched.step(te_loss)  # adjust LR on plateau
        lr = opt.param_groups[0]['lr']
        print(
            f"Epoch {epoch} — "
            f"Train Loss: {tr_loss:.4f} | Train MAE: {tr_mae:.4f} | "
            f"Test Loss: {te_loss:.2f} | Test MAE: {te_mae:.2f} | "
            f"LR: {lr:.5f}"
        )

        # early stopping on Test MAE
        if te_mae < best_te_mae:
            best_te_mae = te_mae
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
    ...
    final_path = sd_dir / f"final-epoch{epoch}-loss{te_loss:.2f}.pth"
    torch.save(model.state_dict(), final_path)

    # ← Make sure to return these lists!
    return train_losses, test_losses, train_maes, test_maes, pit_accs

'''
def run_all(model, ds, crit, opt, sched, device, epochs=epoch_setup):
    # Loop over number of epochs specified

    best_te_mae       = float('inf')  # for early stopping
    epochs_no_improve = 0
    early_stop_patience = early_stop_patience_setup
    ...
    for epoch in range(epochs):
        ...
        # Step LR scheduler based on test loss plateau
        sched.step(te_loss)
...
epochs = epoch_setup
train_losses, test_losses, train_maes, test_maes, pit_accs = run_all(
    model, ds, crit, opt, sched, device, epochs
)
...
###
### Load and evaluate latest model checkpoint ###
###
...
model.eval()
# We position on race no.30 of the 2024 season
ds.set_year(2024)
ds.set_round(12)
...
print("→ batch_inputs → exit shape:", o_batch.shape)
